...
async function startTest(mode, moduleId, moduleName, subjectId, paperId, paperTitle, testId, testTitle) {
  ...
  if (paperId) {
    ...
    // A practice test's own question set is authoritative — don't ALSO
    // require module_id to match. A question's module_id can drift out of
    // sync with its practice_test_id (e.g. edited/reassigned by an admin
    // action that only touched one of the two fields), and requiring both
    // was silently dropping otherwise-correct questions: the test's own
    // count (based on practice_test_id alone, see getQuestionCountsBy)
    // would show e.g. "20 questions", but starting the test could come back
    // with zero — "No questions in this category yet" even though the
    // questions were right there. Same fix, same reasoning as subject_id
    // just below.
    query = query.eq('practice_test_id', testId);
  } else {
    query = query.eq('module_id', moduleId);
    // A subject's own question set is authoritative — don't additionally filter by
    // subjectId's module too, since that could silently drop a question whose subject_id doesn't
...
// Read-only check used by test/paper list screens to show a "Resume" badge on
// the exact card that's paused (see openModuleTestGroup/openSubjectTestGroup/
// openPastPaperCollege in app.js) — same validity rules as checkResumableTest()
// below (not expired, not submitted, not too old) but without popping any
// dialog. Returns the saved snapshot, or null if there's nothing resumable.
export function getResumableSnapshot() {
  ...
  if (saved.timeLimit && elapsed >= saved.timeLimit) return null;
...
function dismissResumableTest() {
  clearPersistedTest();
  document.getElementById('resumeTestBar')?.remove();
  showToast('Resume dismissed');
}
window.dismissResumableTest = dismissResumableTest;
...
function renderTestScreen() {
  ...
  // Review's Finish button is deliberately made to stand out (green glow, not the
  // usual gold) — it's the one action that fully ends the session, as opposed to
  // Pause below which just steps away and comes back to the same spot.
  else finishRowHtml = `<button class="btn btn-finish-highlight" style="margin-bottom:16px" onclick="finishBrowseReview()">✓ Finish Review</button>`;

  // Exit / Pause control — Review (browse) gets a calm "Pause" since nothing is
  // ever lost or graded there; a real Attempt hides this entirely once the
  // student has committed by answering at least one question (see
  // requestExitTest(), which is adjusted to match — no more silent discard once
  // there's something to lose), leaving Finish as the only way to end it early.
  // Practice keeps the original Exit-with-a-choice behavior.
  let exitBtnHtml;
  if (t.mode === 'browse') {
    exitBtnHtml = `<button class="btn btn-secondary" style="width:auto;padding:11px 16px" onclick="requestExitTest()">⏸ Pause</button>`;
  } else if (t.mode === 'attempt' && answered > 0) {
    exitBtnHtml = '';
  } else {
    exitBtnHtml = `<button class="btn btn-danger" style="width:auto;padding:11px 16px" onclick="requestExitTest()">Exit</button>`;
  }
...
function selectOption(idx) {
  ...
  if (t.mode === 'attempt' || t.mode === 'practice' || t.mode === 'browse') persistActiveTest();
  renderTestScreen();
}
...
export function requestExitTest() {
  ...
  if (attempted > 0 && isAttempt) {
    ...
    // a way to walk away without it counting — same rule as the Exit button
    // disappearing from the screen (see renderTestScreen). Leaving mid-test is
    // still possible (closing the app / switching away), and that's still
    // safely resumed via the timer + Profile "Resume" card — this dialog just
    // stops offering a deliberate, no-consequence discard button for it.
    msgEl.innerHTML = `You've answered <strong>${attempted}</strong> of ${t.questions.length} questions, so this attempt now has to be finished one way or another. <strong>Submit Now</strong> ends the test here and counts it in your stats right away. If you just need to step away, close or switch out of the app instead — the timer keeps running and you'll be asked to resume next time you open it.`;
    btnsEl.innerHTML = `
      <button class="btn btn-secondary" onclick="closeModal('modalExitTest')">Keep Going</button>
      <button class="btn btn-primary" onclick="submitAndExitTest()">✓ Submit Now</button>`;
  } else if (attempted > 0) {
    // Practice — still a real choice, since nothing about Practice is ever
    // "locked in" the way a graded Attempt is.
    msgEl.innerHTML = `You've answered <strong>${attempted}</strong> of ${t.questions.length} questions. <strong>Finish Now</strong> ends it here and saves your result. Exiting instead keeps your answers saved so you can pick up right where you left off.`;
    btnsEl.innerHTML = `
      <button class="btn btn-secondary" onclick="closeModal('modalExitTest')">Keep Going</button>
      <button class="btn btn-primary" onclick="submitAndExitTest()">✓ Finish Now</button>
      <button class="btn btn-danger" onclick="forceExitTest()">Exit (Save for Later)</button>`;
  } else {
...
// Quick View, opened from Search — deliberately a FULL SCREEN (reusing the same
// screen-review/reviewPageWrap container as renderReview()'s post-test review),
// not a small popup box. A long explanation made the old popup unreadably
// cramped; this gives it the whole page, a real scroll, and a proper back
// button, exactly like every other review screen in the app.
...
function renderQuickView() {
  ...
  document.getElementById('reviewPageWrap').innerHTML = `
    ...
    <button class="btn btn-bookmark ${st.bookmarked ? 'active' : ''}" onclick="toggleQuickViewBookmark()" style="margin:14px 0">
      ${st.bookmarked ? '🔖 Bookmarked (tap to remove)' : '📖 Bookmark this question'}
    </button>

    ${st.explanationShown ? `
    <div class="explanation-box show">
      <div class="exp-label">✅ Correct Answer: ${esc(opts[q.correct_answer])}</div>
      <div class="exp-content">${q.explanation ? renderMd(q.explanation) : '<span style="color:var(--ink-4)">No explanation added yet.</span>'}</div>
      ${expImgHtml}
      <div class="btn-row" style="margin-top:10px">
        ${isAIEnabled() ? `<button class="btn btn-ghost btn-sm" onclick="openAITutor('${escJs(q.text || '')}','${escJs(q.explanation || '')}')">🤖 Explain with AI</button>` : ''}
        <button class="btn btn-ghost btn-sm" onclick="openReportModal(${q.id})">🚩 Report Question</button>
        <button class="btn btn-ghost btn-sm" onclick="toggleQuickViewExplanation()">🙈 Hide Explanation</button>
      </div>
    </div>
...
function toggleQuickViewExplanation() {
  if (!window._qvState) return;
  window._qvState.explanationShown = !window._qvState.explanationShown;
  renderQuickView();
}
window.toggleQuickViewExplanation = toggleQuickViewExplanation;



async function toggleQuickViewBookmark() {
  const st = window._qvState;
  if (!st) return;
  const q = st.question;
  if (st.bookmarked) {
    ...
    await db(sb.from('bookmarks').upsert({ email: window.currentUser.email, question_id: q.id, added_at: Date.now() }, { onConflict: 'email,question_id' }), 'Bookmark save failed');
    st.bookmarked = true;
    showToast('📖 Bookmarked!');
  }
  renderQuickView();
}
window.toggleQuickViewBookmark = toggleQuickViewBookmark;



// Back button — Search's own screen was never re-rendered under Quick View (we
// only pushed 'review' on top of it), so this just reveals it again as-is,
