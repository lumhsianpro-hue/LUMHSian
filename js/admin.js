...
async function adminPastPapers() {
  ...
  // Grouped by Academic Year → College, same as students now see it (Past
  // Papers → Year → College → Papers) — this is what actually keeps 1st year
  // and 2nd year papers from mixing together. Papers with no year_id yet
  // (from before this field existed) get their own group at the top so
  // they're easy to find and fix — they won't show to any student until
  // they're assigned one via Edit.
  const yearGroups = {};
  ...
  for (const p of papers) {
    ...
    yearGroups[yKey].colleges[cKey].papers.push(p);
  }
  const sortedYearKeys = Object.keys(yearGroups).sort((a, b) => {
    if (a === NO_YEAR) return -1; // surface papers needing attention first
    if (b === NO_YEAR) return 1;
    const ao = yList.findIndex(y => y.id == a), bo = yList.findIndex(y => y.id == b);
    return ao - bo;
  });
  ...
  for (const yKey of sortedYearKeys) {
    ...
    const isNoYear = yKey === NO_YEAR;
    const sortedCollegeKeys = Object.keys(yg.colleges).sort((a, b) => {
      if (a === '__general__') return 1;
      if (b === '__general__') return -1;
      return yg.colleges[a].collegeName.localeCompare(yg.colleges[b].collegeName);
    });
    let collegesHtml = '';
    for (const cKey of sortedCollegeKeys) {
      const cg = yg.colleges[cKey];
      const cTitle = cg.collegeName || 'No College Set';
      collegesHtml += `
        <div style="margin:12px 0 6px;font-size:11px;font-weight:700;color:var(--ink-4);text-transform:uppercase;letter-spacing:.3px">🏫 ${esc(cTitle)} · ${cg.papers.length} paper${cg.papers.length===1?'':'s'}</div>
        ${cg.papers.map((p) => {
          ...
          <div class="flex-between" style="padding:8px 0;border-bottom:1px solid var(--border)">
            <div>
              ...
              <div class="text-xs text-muted">${p.paper_year ? `📅 ${esc(p.paper_year)} · ` : ''}${moduleTag ? `📦 ${esc(moduleTag)} · ` : ''}${qCounts[p.id] || 0} questions</div>
            </div>
    ...
    paperHtml += `<div class="card" style="margin-bottom:10px${isNoYear ? ';border:1.5px solid var(--amber)' : ''}">
      <div class="fw-700">${isNoYear ? '⚠️ No Academic Year Assigned — students won\'t see these yet' : `🎓 ${esc(yg.yearName)}`}</div>
      ${collegesHtml}
    </div>`;
  }
  ...
  document.getElementById('contentTabBody').innerHTML = `
    ...
    <div class="fw-700 mb-2">📜 Past Papers by Academic Year &amp; College <span class="text-xs text-muted" style="font-weight:500">(same grouping students see)</span></div>
    ${paperHtml || '<div class="card"><p class="text-muted">No past papers added yet. Give the paper a name, pick its academic year, then add its questions from the Questions tab.</p></div>'}`;
}
...
function resolveBulkTargetLabel() {
  ...
  const optText = sel?.selectedOptions?.[0]?.textContent?.replace(/\s*\(\d+\s*questions?\)\s*$/i, '').trim();
