...
async function adminPracticeTests() {
  ...
  for (const s of (allSubjects || [])) {
    if (!subjectsByModule[s.module_id]) subjectsByModule[s.module_id] = [];
    subjectsByModule[s.module_id].push(s);
  }
...
async function createPlan() {
  ...
  const { error } = await db(sb.from('subscription_plans').insert({ name, description, price: is_free ? 0 : price, billing_cycle, duration_days, features, is_featured, is_active: false, is_free }), 'Create plan failed');
  if (error) return; // db() already showed the specific error — don't also claim success
...
async function togglePlan(id, current) {
  const { error } = await db(sb.from('subscription_plans').update({ is_active: !current }).eq('id', id), 'Toggle failed');
  if (error) return;
  cacheClear('subscription_plans');
...
// Deleting a plan that a student has ever subscribed to (even just applied
// for — status 'pending') hits subscriptions.plan_id's foreign key, since
// that row still points at this plan. The OLD version here called the
// delete, then showed "Plan deleted" and re-rendered UNCONDITIONALLY,
// without ever checking whether the delete actually succeeded — so a
// blocked delete still said "Plan deleted" while the plan silently stayed
// exactly as it was. This checks for subscribers up front, explains the
// real reason in plain terms, and offers a safe, explicit way to remove
// them together with the plan if that's really what's wanted.
async function deletePlan(id) {
  const { count } = await db(sb.from('subscriptions').select('id', { count: 'exact', head: true }).eq('plan_id', id), 'Check failed');
  if (count > 0) {
    showConfirm(
      `${count} student${count === 1 ? ' has' : 's have'} a subscription tied to this plan, so it can't be deleted on its own. Delete ${count === 1 ? 'that subscription' : 'those subscriptions'} together with the plan? This cannot be undone.`,
      async () => {
        const { error: subErr } = await db(sb.from('subscriptions').delete().eq('plan_id', id), 'Removing subscriptions failed');
        if (subErr) return;
        const { error } = await db(sb.from('subscription_plans').delete().eq('id', id), 'Delete failed');
        if (error) return;
        cacheClear('subscription_plans');
        showToast('Plan and its subscription(s) deleted');
        adminMonetization();
      },
      'Delete Anyway'
    );
...
// Approve now updates `subscriptions` directly instead of going through the
// admin_approve_subscription RPC. Admins already have UPDATE rights on this
// table via RLS ("subscriptions_update_admin_only"), so the RPC was an extra
// layer that wasn't strictly needed — and it's server-side SQL that lives in
// the database itself, not in any of these files, so whatever bug was inside
// it (this call was reporting "approved" without the row actually changing)
// isn't something that could be inspected or fixed from here. This path is
// fully visible: if it fails, `error` is checked and a real failure message
// shows, and if it succeeds, the row is genuinely updated.
async function approveSubscription(id) {
  ...
  const { error } = await db(sb.from('subscriptions').update({ status: 'active', approved_at: new Date().toISOString(), expires_at: expiry }).eq('id', id), 'Approve failed');
  if (error) return;
