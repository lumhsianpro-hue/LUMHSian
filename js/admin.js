...
async function adminPastPapers() {
        ...
        ${yg.papers.map((p) => {
          ...
          <div class="flex-between" style="padding:8px 0;border-bottom:1px solid var(--border)">
            ...
            <div style="display:flex;gap:6px;flex-wrap:wrap;justify-content:flex-end">
              <button class="btn btn-secondary btn-xs" onclick="startReplaceAllQuestions('paper',${p.id},'${escJs(p.title)}',${qCounts[p.id] || 0})">🔁 Replace Qs</button>
  ...
  document.getElementById('contentTabBody').innerHTML = `
    ...
    <div class="card" style="margin-bottom:20px;border:1.5px solid var(--gold-400)">
      ...
      <p class="text-xs text-muted mb-3">Create many paper entries at once from an Excel/CSV file — this only creates the papers themselves (title, college, year, module tags). Add each paper's questions afterward from Content → Questions → Bulk Upload, picking the paper there.</p>
...
function downloadPaperTemplate() {
  const wb = XLSX.utils.book_new();
  const ws = XLSX.utils.json_to_sheet([
    { Title: 'Annual Exam 2023', CollegeName: 'LUMHS Jamshoro', PaperYear: '2023', Modules: 'Anatomy, Physiology', DisplayOrder: 1, IsActive: 'TRUE' },
    { Title: 'Supplementary 2022', CollegeName: 'LUMHS Jamshoro', PaperYear: '2022 Supplementary', Modules: 'Biochemistry', DisplayOrder: 2, IsActive: 'TRUE' }
  ]);
  ...
  reader.onload = (e) => {
    try {
      ...
      ppBulkData = rows.map(row => ({
        ...
        paper_year: String(norm(row, 'PaperYear', 'Year')).trim() || null,
      ...
      document.getElementById('ppBulkPreview').innerHTML = `
        ...
        ${ppBulkData.slice(0, 5).map(p => `<div class="card" style="margin-bottom:6px;font-size:13px"><strong>${esc(p.title)}</strong> · ${esc(p.college_name)}${p.paper_year ? ` · ${esc(p.paper_year)}` : ''}</div>`).join('')}
...
async function executePaperBulkUpload() {
  ...
  const [{ data: existingColleges }, { data: existingModules }] = await Promise.all([
  ...
  let success = 0, failed = 0, tagsLinked = 0, tagsSkipped = 0, collegeSkipped = 0, firstErr = '';
  for (const p of ppBulkData) {
    // Same rule as the single Add Past Paper form: the college must already exist
    // (Content → Colleges) so students' College → Year browse tree stays consistent
    // instead of fragmenting into near-duplicate spellings.
    if (!collegeNames.has(p.college_name.toLowerCase())) {
      collegeSkipped++;
      continue;
    }
    const { data: inserted, error } = await sb.from('past_papers').insert({
      title: p.title, college_name: p.college_name, paper_year: p.paper_year,
  ...
  showToast(msg, (failed || tagsSkipped || collegeSkipped) ? 12000 : 4000);
    ...
    <div style="background:var(--surface);border-radius:var(--radius-xl);padding:24px;width:100%;max-width:420px;max-height:90vh;overflow-y:auto">
      ...
      <div class="btn-row mt-3">
        ...
        <button class="btn btn-primary" onclick="(async()=>{
          ...
          const r1=await db(sb.from('past_papers').update({title:t,college_name:c,paper_year:y,display_order:o,is_active:a}).eq('id',${p.id}),'Edit failed — title/college/year not saved');
          if(r1.error){return;}
          const r2=await db(sb.from('past_paper_modules').delete().eq('paper_id',${p.id}),'Tag update failed — module tags not saved');
          if(r2.error){return;}
          if (moduleIds.length) { const r3=await db(sb.from('past_paper_modules').insert(moduleIds.map(module_id=>({paper_id:${p.id},module_id}))),'Tag save failed — module tags not saved'); if(r3.error){return;} }
          showToast('Past paper updated ✓');
          this.closest('[style*=fixed]').remove();
          adminPastPapers();
          } catch(err) { console.error('Edit past paper failed:', err); showToast('Unexpected error: '+err.message, 9000); }
        })()">Save Changes</button>
      </div>
    </div>`;
  document.body.appendChild(overlay);
    ...
    <div style="background:var(--surface);border-radius:var(--radius-xl);padding:24px;width:100%;max-width:420px;max-height:90vh;overflow-y:auto">
      ...
      <div class="btn-row mt-3">
        ...
        <button class="btn btn-primary" onclick="(async()=>{
          ...
          const r1=await db(sb.from('practice_tests').update({title:ti,subject_id:s,display_order:o,is_active:a}).eq('id',${t.id}),'Edit failed');
          if(r1.error){return;}
          showToast('Practice test updated ✓');
          this.closest('[style*=fixed]').remove();
          adminPracticeTests();
          } catch(err) { console.error('Edit practice test failed:', err); showToast('Unexpected error: '+err.message, 9000); }
        })()">Save Changes</button>
      </div>
    </div>`;
  document.body.appendChild(overlay);
  ...
  return `
    <div class="card" style="margin-bottom:20px">
      ...
      <div id="qx_paperFields" style="display:${mode==='paper'?'block':'none'}">
        <label class="input-label">Past Paper <span class="text-xs text-muted">(e.g. "Dow: Head &amp; Neck 2020")</span></label>
...
async function loadSubjectsForQX() {
  ...
  if (papSel) papSel.innerHTML = '<option value="" disabled selected>Select past paper...</option>';
...
async function loadPapersForQ() {
  ...
  if (!papSel) return;
  papSel.innerHTML = '<option value="" disabled selected>Select past paper...</option>';
  // Past papers aren't tied to one module anymore (a real paper spans several
