...
import { _debounce, cacheClear, esc, renderMd, showConfirm, showLoading, showToast } from './utils.js';
...
async function adminEditYear(id) {
  ...
  overlay.innerHTML = `
    <div style="background:var(--surface);border-radius:var(--radius-xl);padding:24px;width:100%;max-width:420px">
      ...
      <label class="input-label">Year Name</label>
      <input id="_ey_name" class="input-field" value="${y.name.replace(/"/g,'&quot;')}">
      <label class="input-label">Display Order</label>
      ...
      <input id="_ey_coming" class="input-field" placeholder="e.g., Available soon!" value="${(y.coming_soon_text||'').replace(/"/g,'&quot;')}">
      <div class="btn-row mt-3">
        <button class="btn btn-ghost" onclick="this.closest('[style*=fixed]').remove()">Cancel</button>
        <button class="btn btn-primary" onclick="(async()=>{
          ...
          await db(sb.from('years').update({name:n,display_order:o,is_active:a,coming_soon_text:c}).eq('id',${y.id}),'Edit failed');
          cacheClear('years');
...
async function adminEditModule(id) {
  ...
  overlay.innerHTML = `
    <div style="background:var(--surface);border-radius:var(--radius-xl);padding:24px;width:100%;max-width:420px;max-height:90vh;overflow-y:auto">
      ...
      <label class="input-label">Module Name</label>
      <input id="_em_name" class="input-field" value="${m.name.replace(/"/g,'&quot;')}">
      <label class="input-label">Description</label>
      <textarea id="_em_desc" class="input-field" rows="2" placeholder="Brief description" style="resize:vertical">${m.description||''}</textarea>
      <label class="input-label">Image URL</label>
      <input id="_em_img" class="input-field" placeholder="Image URL" value="${m.icon_url||''}">
      ...
      <div class="btn-row mt-3">
        ...
        <button class="btn btn-primary" onclick="(async()=>{
          ...
          const img=document.getElementById('_em_img').value.trim()||m.icon_url||null;
          const c=document.getElementById('_em_color').value;
          const cat=document.getElementById('_em_cat').value;
          if(!n)return showToast('Name required');
          await db(sb.from('modules').update({name:n,description:d,icon_url:img,color:c,category:cat}).eq('id',${m.id}),'Edit failed');
...
async function adminDeleteModule(id) {
  showConfirm('Delete this module?<br><small>All subjects, past papers, practice tests and questions inside will also be permanently deleted.</small>', async () => {
    ...
    await db(sb.from('past_papers').delete().eq('module_id', id), 'Papers delete failed');
...
async function adminEditSubject(id, oldName, oldOrder) {
  ...
  overlay.innerHTML = `
    <div style="background:var(--surface);border-radius:var(--radius-xl);padding:24px;width:100%;max-width:400px">
      ...
      <label class="input-label">Subject Name</label>
      <input id="_es_name" class="input-field" value="${oldName.replace(/"/g,'&quot;')}">
      <label class="input-label">Display Order</label>
      ...
      <div class="btn-row mt-3">
        ...
        <button class="btn btn-primary" onclick="(async()=>{
          ...
          if(!n)return showToast('Name required');
          await db(sb.from('subjects').update({name:n,display_order:o}).eq('id',${id}),'Edit failed');
...
async function adminPastPapers() {
  ...
  const pp_college_options = `<option value="" disabled selected>Select college...</option>` +
    (collegeRows || []).map(c => `<option value="${c.name.replace(/"/g,'&quot;')}">${c.name}</option>`).join('');
  ...
  // Grouped by College → Year, same as students now see it under the Past
  // Papers card, instead of the old per-module grouping — modules are still
  // shown as a small tag per paper (any number, not capped at 2) since a
  // college+year group here can span several modules.
  const collegeGroups = {};
  for (const p of papers) {
    const collegeName = (p.college_name || '').trim();
    const cKey = collegeName || '__general__';
    if (!collegeGroups[cKey]) collegeGroups[cKey] = { collegeName, years: {} };
    const yearName = (p.paper_year || '').trim();
    const yKey = yearName || '__noyear__';
    if (!collegeGroups[cKey].years[yKey]) collegeGroups[cKey].years[yKey] = { year: yearName, papers: [] };
    collegeGroups[cKey].years[yKey].papers.push(p);
  }
  const sortedCollegeKeys = Object.keys(collegeGroups).sort((a, b) => {
  ...
  for (const cKey of sortedCollegeKeys) {
    const cg = collegeGroups[cKey];
    const cTitle = cg.collegeName || 'No College Set';
    const sortedYearKeys = Object.keys(cg.years).sort((a, b) => {
    ...
    for (const yKey of sortedYearKeys) {
      const yg = cg.years[yKey];
      yearsHtml += `
        <div style="margin:12px 0 6px;font-size:11px;font-weight:700;color:var(--ink-4);text-transform:uppercase;letter-spacing:.3px">${esc(yg.year || 'No Year Set')} · ${yg.papers.length} paper${yg.papers.length===1?'':'s'}</div>
        ${yg.papers.map((p) => {
          const moduleTag = (moduleIdsByPaper[p.id] || []).map(id => moduleNameById[id]).filter(Boolean).join(' + ');
          return `
          <div class="flex-between" style="padding:8px 0;border-bottom:1px solid var(--border)">
            <div>
              <div class="text-sm fw-600">${esc(p.title)} ${p.is_active ? '' : '<span class="badge badge-amber" style="font-size:9px">Hidden</span>'}</div>
              <div class="text-xs text-muted">${moduleTag ? `📦 ${esc(moduleTag)} · ` : ''}${qCounts[p.id] || 0} questions</div>
            </div>
            <div style="display:flex;gap:6px">
    ...
    paperHtml += `<div class="card" style="margin-bottom:10px">
      <div class="fw-700">🏫 ${esc(cTitle)}</div>
      ${yearsHtml}
    </div>`;
  }
  ...
  document.getElementById('contentTabBody').innerHTML = `
    <div class="card" style="margin-bottom:20px">
      ...
      <label class="input-label">College / University <span class="text-xs text-muted">(this is what students browse by — Past Papers → College → Year)</span></label>
    ...
    <div class="fw-700 mb-2">📜 Past Papers by College &amp; Year <span class="text-xs text-muted" style="font-weight:500">(same grouping students see)</span></div>
    ${paperHtml || '<div class="card"><p class="text-muted">No past papers added yet. Give the paper a name (e.g. its college/university and year), then add its questions from the Questions tab.</p></div>'}`;
}
...
async function adminAddPastPaper() {
  ...
  const { data: inserted } = await db(sb.from('past_papers').insert({ title, college_name, paper_year, display_order, is_active }).select('id').single(), 'Add failed');
...
async function adminEditPastPaper(id) {
  const [{ data: p }, { data: collegeRows }, { data: modules }, { data: existingTags }] = await Promise.all([
  ...
  for (const c of (collegeRows || [])) {
    ...
    ep_college_options += `<option value="${c.name.replace(/"/g,'&quot;')}" ${c.name === currentCollege ? 'selected' : ''}>${c.name}</option>`;
  }
  if (currentCollege && !matched) {
    ep_college_options += `<option value="${currentCollege.replace(/"/g,'&quot;')}" selected>${currentCollege} (not in list)</option>`;
  }
  ...
  overlay.innerHTML = `
    <div style="background:var(--surface);border-radius:var(--radius-xl);padding:24px;width:100%;max-width:420px;max-height:90vh;overflow-y:auto">
      ...
      <input id="_ep_title" class="input-field" value="${(p.title||'').replace(/"/g,'&quot;')}">
      ...
      <label class="input-label">Paper Year / Session</label>
      <input id="_ep_year" class="input-field" value="${(p.paper_year||'').replace(/"/g,'&quot;')}">
      <label class="input-label">Display Order</label>
      ...
      <div class="btn-row mt-3">
        ...
        <button class="btn btn-primary" onclick="(async()=>{
          ...
          await db(sb.from('past_papers').update({title:t,college_name:c,paper_year:y,display_order:o,is_active:a}).eq('id',${p.id}),'Edit failed');
          await db(sb.from('past_paper_modules').delete().eq('paper_id',${p.id}),'Tag update failed');
          if (moduleIds.length) await db(sb.from('past_paper_modules').insert(moduleIds.map(module_id=>({paper_id:${p.id},module_id}))),'Tag save failed');
          showToast('Past paper updated ✓');
          this.closest('[style*=fixed]').remove();
          adminPastPapers();
        })()">Save Changes</button>
      </div>
...
async function adminEditPracticeTest(id) {
  ...
  overlay.innerHTML = `
    <div style="background:var(--surface);border-radius:var(--radius-xl);padding:24px;width:100%;max-width:420px;max-height:90vh;overflow-y:auto">
      ...
      <input id="_ept_title" class="input-field" value="${(t.title||'').replace(/"/g,'&quot;')}">
      ...
      <select id="_ept_subject" class="input-field" title="Subject" aria-label="Subject">
        ...
        ${(subs||[]).map(s=>`<option value="${s.id}" ${s.id===t.subject_id?'selected':''}>${s.name}</option>`).join('')}
      </select>
      <label class="input-label">Display Order</label>
      ...
      <div class="btn-row mt-3">
        ...
        <button class="btn btn-primary" onclick="(async()=>{
          ...
          if(!ti)return showToast('Title required');
          await db(sb.from('practice_tests').update({title:ti,subject_id:s,display_order:o,is_active:a}).eq('id',${t.id}),'Edit failed');
...
function qSubTab(tab, btn) {
  ...
  if (tab === 'add') {
    document.getElementById('qTabBody').innerHTML = `
      <div class="card">
        <label class="input-label">Module <span class="text-xs text-muted">(required, unless you pick a Past Paper below — then optional)</span></label>
        ...
        <label class="input-label">Past Paper (optional, only if this question belongs to a specific past paper)</label>
    ...
    loadPapersForQ(); // populates the Past Paper dropdown right away — it no longer depends on a module being picked first
  } else if (tab === 'browse') {
    document.getElementById('qTabBody').innerHTML = `
      <div class="card" style="margin-bottom:20px">
    ...
    document.getElementById('qTabBody').innerHTML = `
      <div class="card" style="margin-bottom:20px">
        <div class="fw-700 mb-2">🎯 Step 1: Which Module/Subject/Paper are these questions for</div>
        <p class="text-sm mb-3">This selection is shared by all three upload methods below (AI Text, Excel, JSON). Just set it once.</p>
        <label class="input-label">Module <span class="text-xs text-muted">(required, unless you pick a Past Paper below — then optional)</span></label>
        ...
        <label class="input-label">Past Paper (optional, pick this if all questions belong to one past paper, e.g. "Dow: Head &amp; Neck 2020")</label>
    ...
    loadSubjectsForQX(); // populates the Past Paper dropdown right away — it no longer depends on a module being picked first
  }
}
...
async function loadSubjectsForQX() {
  ...
  const papSel = document.getElementById('qx_paper');
  const testSel = document.getElementById('qx_test');
  if (subSel) subSel.innerHTML = '<option value="">Any / Mixed</option>';
  if (papSel) papSel.innerHTML = '<option value="">Not part of a past paper</option>';
  ...
  // subjects) — this list is independent of whichever module is selected
  // above, so it loads regardless and shows every paper, grouped by college.
  ...
  if (papSel) papSel.innerHTML += pastPapersOptgroupHtml(papers, null, paperQCounts);
...
function previewBulkExcel() {
  ...
  if (!file) return;
  if (!document.getElementById('qx_module').value && !document.getElementById('qx_paper')?.value) {
    showToast('Select a Module, or a Past Paper, in Step 1 first');
...
async function executeBulkExcelUpload() {
  ...
  const moduleId = document.getElementById('qx_module').value || null;
  const subjectId = document.getElementById('qx_subject').value || null;
  const paperId = document.getElementById('qx_paper')?.value || null;
  const testId = document.getElementById('qx_test')?.value || null;
  // A past paper's questions don't need a shared module — a real paper mixes
  // subjects, so this only blocks when NEITHER a module nor a paper is set.
  if (!moduleId && !paperId) return showToast('Select a Module, or a Past Paper, in Step 1 first');
  ...
  showLoading(false);
  showToast(`✅ ${success} added${failed ? `, ❌ ${failed} failed: ${firstErr}` : ''}${imagesMissing ? `, ⚠️ ${imagesMissing} image(s) not matched` : ''}`, failed ? 12000 : 3000);
...
function previewAIQuestions() {
  const moduleId = document.getElementById('qx_module')?.value;
  const paperId = document.getElementById('qx_paper')?.value;
  // A past paper's questions don't need a shared module — a real paper mixes
  // subjects, so this only blocks when NEITHER a module nor a paper is set.
  if (!moduleId && !paperId) { showToast('Select a Module, or a Past Paper, in Step 1 first'); return; }
...
async function executeAIQuestionUpload() {
  ...
  showLoading(false);
  showToast(`✅ ${success} added${failed ? `, ❌ ${failed} failed: ${firstErr}` : ''}`, failed ? 12000 : 3000);
...
async function executeBulkUpload() {
  ...
  for (const q of bulkQData) {
    const { error } = await sb.from('questions').insert({
      module_id: q.module_id, subject_id: q.subject_id || null, paper_id: q.paper_id || null, practice_test_id: q.practice_test_id || null,
      text: q.text, options: q.options, correct_answer: q.correct_answer,
...
function pastPapersOptgroupHtml(papers, selectedId, counts) {
  ...
  for (const c of collegeNames) {
    ...
    for (const p of ps) {
      ...
      html += `<option value="${p.id}" ${selectedId && p.id === selectedId ? 'selected' : ''}>${yearPrefix}${esc(p.title)}${countSuffix}</option>`;
    }
    html += `</optgroup>`;
  }
  return html;
...
async function loadPapersForQ() {
  const papSel = document.getElementById('q_paper');
  if (!papSel) return;
  papSel.innerHTML = '<option value="">Not part of a past paper</option>';
  // Past papers aren't tied to one module anymore (a real paper spans several
  // subjects) — this is always the full list, independent of the module
  // selected above.
  const { data: papers } = await db(sb.from('past_papers').select('id,title,college_name,paper_year').order('display_order'), 'Papers error');
  const paperQCounts = await getQuestionCountsBy('paper_id', (papers || []).map(p => p.id));
  papSel.innerHTML += pastPapersOptgroupHtml(papers, null, paperQCounts);
}
...
async function onQModuleChange() {
  // Reset subject/paper/test selections immediately so stale values can't persist
  const subSel = document.getElementById('q_subject');
  const papSel = document.getElementById('q_paper');
  const testSel = document.getElementById('q_test');
  if (subSel) subSel.value = '';
  if (papSel) papSel.value = '';
  if (testSel) testSel.value = '';
  await Promise.all([loadSubjectsForQ(), loadPapersForQ(), loadTestsForQ()]);
}
...
async function adminAddQuestion() {
  const module_id = document.getElementById('q_module')?.value || null;
  const subject_id = document.getElementById('q_subject')?.value || null;
  const paper_id = document.getElementById('q_paper')?.value || null;
  const practice_test_id = document.getElementById('q_test')?.value || null;
  ...
  // Module is required UNLESS this question belongs to a Past Paper — a real
  // past paper mixes modules/subjects, so a paper question doesn't need one.
  if (!module_id && !paper_id) return showToast('Select a Module, or a Past Paper, before adding a question');
    ...
    <div style="background:var(--surface);border-radius:var(--radius-xl);padding:24px;width:100%;max-width:460px;max-height:90vh;overflow-y:auto">
      ...
      <div class="btn-row mt-3">
        ...
        <button class="btn btn-primary" onclick="(async()=>{
          ...
          if(!t)return showToast('Question text required');
          await db(sb.from('questions').update({text:t,explanation:e||null,correct_answer:c,subject_id:subj,practice_test_id:test,paper_id:pap,image_url:img,explanation_image_url:expImg}).eq('id',${id}),'Edit failed');
...
function adminViewAsStudent() {
  ...
  retBtn.innerHTML = `<span>👁 Admin Preview Mode</span><button onclick="exitAdminPreview()" style="background:rgba(255,255,255,.2);border:none;color:white;padding:6px 14px;border-radius:999px;font-size:12px;font-weight:700;cursor:pointer">← Back to Admin</button>`;
...
async function editPlan(id) {
  ...
  overlay.innerHTML = `
    <div style="background:var(--surface);border-radius:var(--radius-xl);padding:24px;width:100%;max-width:420px;max-height:90vh;overflow-y:auto">
      ...
      <label class="input-label">Plan Name</label>
      <input id="_ep_name" class="input-field" value="${p.name.replace(/"/g,'&quot;')}">
      <label class="input-label">Price (PKR)</label>
      <input id="_ep_price" type="number" class="input-field" title="Plan price" aria-label="Plan price" value="${p.price||0}">
      <label class="input-label">Duration (days)</label>
      <input id="_ep_days" type="number" class="input-field" title="Duration in days" aria-label="Duration in days" value="${p.duration_days||30}">
      <label class="input-label">Description</label>
      <input id="_ep_desc" class="input-field" value="${(p.description||'').replace(/"/g,'&quot;')}">
      <label class="input-label">Features (comma separated)</label>
      <textarea id="_ep_feat" class="input-field" rows="3" style="resize:vertical">${(p.features||[]).join(', ')}</textarea>
      <div class="btn-row mt-3">
        <button class="btn btn-ghost" onclick="this.closest('[style*=fixed]').remove()">Cancel</button>
        <button class="btn btn-primary" onclick="(async()=>{
          const n=document.getElementById('_ep_name').value.trim();
          const pr=parseFloat(document.getElementById('_ep_price').value)||0;
          const dy=parseInt(document.getElementById('_ep_days').value)||30;
          const d=document.getElementById('_ep_desc').value.trim();
          const f=document.getElementById('_ep_feat').value.split(',').map(x=>x.trim()).filter(Boolean);
          if(!n)return showToast('Plan name required');
          await db(sb.from('subscription_plans').update({name:n,price:pr,description:d,duration_days:dy,features:f}).eq('id',${p.id}),'Edit failed');
          cacheClear('subscription_plans');
