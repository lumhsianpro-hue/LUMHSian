      ...
      module: () => {
        if (window._moduleScreenMode === 'pastpapers' && window._lastOpenedPastPapers) {
          ...
          openPastPapersRoot().then(() => {
            if (pp.level === 'college' && pp.yearId && pp.collegeKey) { openPastPapersYear(pp.yearId, pp.yearName); openPastPaperCollege(pp.collegeKey); }
            else if (pp.level === 'year' && pp.yearId) openPastPapersYear(pp.yearId, pp.yearName);
          });
