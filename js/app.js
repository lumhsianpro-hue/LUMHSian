...
function selectYear(id, name) {
  ...
  if (window.currentUser) {
    ...
    // Skip persisting to the DB while an admin is previewing as a student —
    // otherwise every year they jump to in Preview mode overwrites their own
    // real account's year_of_study, which is meaningless churn on an admin row.
    if (!window._isAdminPreview) {
...
export async function renderHome() {
  ...
  wrap.innerHTML = `
    ...
    ${localStorage.getItem(RESUME_KEY) ? `<div id="resumeTestBar" style="display:flex;gap:8px;align-items:center;margin-bottom:16px">
      <button class="btn btn-secondary btn-sm" style="flex:1" onclick="checkResumableTest()">▶ Resume Test</button>
...
// ==================== GLOBAL PAST PAPERS (Academic Year → College → Papers) ====================
// Scoped by Academic Year first (so a 1st year student's list never mixes in
// 2nd year papers, etc.) — same "years" table Modules already use — then by
// College within that year. Reuses the 'module' screen/modulePageWrap the
// same in-place way openModule()'s old sub-views did, so Review/Solve still
// hand off to the exact same startTest() used everywhere else — driven by
// paper_id alone (see startTest()), since a single paper can still be tagged
// with any number of modules via the past_paper_modules table (e.g. one exam
// paper covering GIT + Endocrinology + Pharmacology), purely as a display
// label, independent of which Academic Year the paper itself belongs to.
let _pastPapersData = null;

 // { yearGroups, yearOrder } — cached per visit so drilling down/back never re-fetches
...
async function openPastPapersRoot() {
  ...
  const yearNameById = {};
  for (const y of (allYears || [])) yearNameById[y.id] = y.name;
  // Papers with no Academic Year assigned yet (from before this field existed)
  // don't show here — see adminPastPapers(), which flags these clearly so
  // they're quick to fix with one Edit each.
  const papers = (allPapers || []).filter(p => p.year_id);
  const ppCounts = await getQuestionCountsBy('paper_id', papers.map(p => p.id));
  showLoading(false);

  // Grouped by Academic Year first, then College — this is what keeps 1st
  // year and 2nd year (etc.) papers from ever mixing together. Papers with
  // zero questions are kept out entirely, same rule the old grouping used.
  // Question count + module tag are stashed on each paper (_qCount/
  // _moduleTag) so drilling down never needs to re-fetch either. _moduleTag
  // joins every module this paper is tagged with via past_paper_modules (any
  // number, e.g. "GIT + Endocrinology + Pharmacology") — it's purely a
  // display label admin sets, not a real relationship, and is blank if
  // nothing is tagged.
  const yearGroups = {};
  for (const p of papers) {
    const qCount = ppCounts[p.id] || 0;
    if (!qCount) continue;
    const yearName = yearNameById[p.year_id];
    if (!yearName) continue; // year was deleted after this paper was tagged
    if (!yearGroups[p.year_id]) yearGroups[p.year_id] = { yearName, colleges: {} };
    const collegeName = (p.college_name || '').trim();
    const cKey = collegeName || '__general__';
    if (!yearGroups[p.year_id].colleges[cKey]) yearGroups[p.year_id].colleges[cKey] = { collegeName, papers: [] };
    const moduleTag = (moduleIdsByPaper[p.id] || []).map(id => moduleNameById[id]).filter(Boolean).join(' + ');
    yearGroups[p.year_id].colleges[cKey].papers.push({ ...p, _qCount: qCount, _moduleTag: moduleTag });
  }
  _pastPapersData = { yearGroups, yearOrder: (allYears || []).map(y => y.id) };
...
// Renders the Academic Year list shell (header + list container). This is
// Level 1 — every year shows up (own year included), same as Modules already
// do on Home, so browsing another year's papers is still possible, but
// Solve/Review on them stay locked (see openPastPaperCollege below).
function renderPastPapersRootShell() {
  ...
  wrap.innerHTML = `
    <button class="back-btn" onclick="goBack()">← Back</button>
    <div class="card-teal" style="margin-bottom:16px"><h2>${ICON_BUILDING} Past Papers</h2><p>Choose your academic year</p></div>
    <div id="ppYearListInner"></div>
    <div style="height:16px"></div>`;
  renderPastPapersYearList();
}
window.renderPastPapersRootShell = renderPastPapersRootShell;



function renderPastPapersYearList() {
  const listWrap = document.getElementById('ppYearListInner');
  if (!listWrap || !_pastPapersData) return;
  const { yearGroups, yearOrder } = _pastPapersData;
  const myYearName = window.currentUser?.year_of_study || null;

  let html = '';
  for (const yearId of yearOrder) {
    const g = yearGroups[yearId];
    if (!g) continue; // this year has no papers with questions yet
    const collegeCount = Object.keys(g.colleges).length;
    const paperCount = Object.values(g.colleges).reduce((sum, c) => sum + c.papers.length, 0);
    const isMine = g.yearName === myYearName;
    html += `
      <div class="list-item" onclick="openPastPapersYear(${yearId},'${escJs(g.yearName)}')">
        <div class="list-item-left">
          <div class="list-item-icon">${ICON_CALENDAR}</div>
          <div style="min-width:0">
            <div class="list-item-title">${esc(g.yearName)} Past Papers${isMine ? ' <span style="margin-left:6px;font-size:11px;font-weight:700;color:#1a1a1a;background:var(--gold-500);padding:2px 8px;border-radius:999px">Your Year</span>' : ''}</div>
            <div class="list-item-sub">${collegeCount} college${collegeCount === 1 ? '' : 's'} · ${paperCount} paper${paperCount === 1 ? '' : 's'}</div>
          </div>
        </div>
        <span style="color:var(--ink-4)">›</span>
      </div>`;
  }
  listWrap.innerHTML = html || `<div class="card"><p>No past papers added yet. Check back soon.</p></div>`;
}
window.renderPastPapersYearList = renderPastPapersYearList;



// Academic Year tapped from the root list — Level 2, every college that has
// papers for that year. Browsing a year other than the student's own is
// allowed (banner explains it), same as Modules — only Solve/Review lock.
function openPastPapersYear(yearId, yearName) {
  const g = _pastPapersData?.yearGroups?.[yearId];
  if (!g) { openPastPapersRoot(); return; }
  window._lastOpenedPastPapers = { level: 'year', yearId, yearName };
  window._moduleScreenMode = 'pastpapers';
  const isOwnYear = _isYearContentUnlocked(yearName);

  const wrap = document.getElementById('modulePageWrap');
  wrap.innerHTML = `
    <button class="back-btn" onclick="renderPastPapersRootShell()">← Back to Past Papers</button>
    <div class="card-teal" style="margin-bottom:16px"><h2>${ICON_CALENDAR} ${esc(yearName)}</h2><p>Choose a college</p></div>
    ${!isOwnYear ? `<div class="card" style="margin-bottom:16px;background:linear-gradient(135deg,var(--amber-50,#fff7e6) 0%,var(--surface) 100%);border:1px solid var(--amber)"><p class="text-sm">🔒 You're browsing ${esc(yearName)}'s past papers — you can look through the list, but Solve/Review stay locked to your own year.</p></div>` : ''}
...
window.openPastPapersYear = openPastPapersYear;
...
function filterPastPaperColleges(term) {
  ...
  const yearId = window._lastOpenedPastPapers?.yearId;
  const g = _pastPapersData?.yearGroups?.[yearId];
  if (!listWrap || !g) return;
  const t = (term || '').trim().toLowerCase();

  const sortedKeys = Object.keys(g.colleges).sort((a, b) => {
  ...
  listWrap.innerHTML = html || `<div class="card"><p>${t ? 'No colleges match your search.' : 'No colleges yet for this year.'}</p></div>`;
}
window.filterPastPaperColleges = filterPastPaperColleges;



// College tapped from a year's college list — Level 3, every actual paper
// for that year+college, flat (papers aren't tied to one module, so there's
// nothing to section by). Review/Solve wire into the exact same startTest()
// as everywhere else, which pulls this paper's questions by paper_id alone
// (module is passed as null below — it's not read in paper mode), but are
// locked behind the same wrong-year rule Modules use whenever this isn't the
// student's own academic year.
async function openPastPaperCollege(collegeKey) {
  const { yearId, yearName } = window._lastOpenedPastPapers || {};
  const g = _pastPapersData?.yearGroups?.[yearId]?.colleges?.[collegeKey];
  if (!g) { openPastPapersRoot(); return; }
  window._lastOpenedPastPapers = { level: 'college', yearId, yearName, collegeKey };
  window._moduleScreenMode = 'pastpapers';
  const title = g.collegeName || 'Other Colleges';
  const papers = [...g.papers].sort((a, b) => (a.title || '').localeCompare(b.title || ''));
  const isOwnYear = _isYearContentUnlocked(yearName);
  const lockedClick = `blockWrongYear('${(yearName||'').replace(/'/g,"\\'")}')`;
    ...
    html += `
      <div class="list-item no-hover" style="flex-direction:column;align-items:stretch;cursor:default">
        ...
        <div class="btn-row mt-2">
          <button class="btn btn-secondary btn-sm" onclick="${isOwnYear ? `startTest('browse',null,'${escJs(p.title)}',null,${p.id},'${escJs(p.title)}')` : lockedClick}">👁 Review</button>
          <button class="btn btn-primary btn-sm" onclick="${isOwnYear ? `startTest('attempt',null,'${escJs(p.title)}',null,${p.id},'${escJs(p.title)}')` : lockedClick}">📝 Solve</button>
        </div>
      </div>`;
  }
  ...
  const wrap = document.getElementById('modulePageWrap');
  wrap.innerHTML = `
    <button class="back-btn" onclick="openPastPapersYear(${yearId},'${escJs(yearName)}')">← Back to ${esc(yearName)}</button>
    ...
    const renders = {
      ...
      module: () => {
        if (window._moduleScreenMode === 'pastpapers') {
          // Deliberately always resume to the Year list (Level 1), never deep
          // into a specific year/college — replaying an old saved path here
          // used to show stale data (e.g. still-2nd-year papers) whenever the
          // admin/student's effective year had changed since the last visit.
          // Past Papers is a "pick your year" flow by design, so restarting
          // at the picker is correct, not a downgrade.
          openPastPapersRoot();
  ...
  try {
    ...
    if (state.screenId === 'module') {
      if (state.pastPapersState) {
        // Deliberately resume to the Year list (Level 1) only, not deep into
        // a saved year/college — see the tab-switch resume handler above for
        // why (stale years can otherwise reappear after a year change).
